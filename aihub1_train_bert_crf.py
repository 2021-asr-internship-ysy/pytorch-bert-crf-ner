...
def main(parser):
    ...
    # data_config = Config(json_path=data_dir / 'aihub1_config.json') 
    model_config = Config(json_path=model_dir / 'aihub1_config.json') #config 수정
    print("checkpoint1")

    # Vocab & Tokenizer
    tok_path = get_tokenizer() # ./tokenizer_78b3253a26.model
    ptr_tokenizer = SentencepieceTokenizer(tok_path)
    #ptr_tokenizer = "/home/hoya/notebooks/syoung21.yoo/pytorch-bert-crf-ner/bert1_aihub/tokenization_morp.py"
    ...
    print("len(token_to_idx): ", model_config.vocab_size)

    """dump 코드 필요없음
    with open(model_dir / "token2idx_aihub1.json", 'w', encoding='utf-8') as f: #vocab2number 수정
        json.dump(token_to_idx, f, ensure_ascii=False, indent=4)


    # save vocab & tokenizer
    with open(model_dir / "vocab_aihub1.pkl", 'wb') as f: #vocab 수정
        pickle.dump(vocab, f)
    """

    # load vocab & tokenizer
    with open(model_dir / "vocab_aihub1.pkl", 'rb') as f: #vocab 수정
        vocab = pickle.load(f)
    ...
    # Model
    model = KobertAihubCRF(config=model_config, num_classes=len(tr_ds.ner_to_index))
    model.to(device)
    model.train()
    ...
    print("  Num examples = %d", len(tr_ds))
    print("  Num Epochs = %d", model_config.epochs)
    print("  Instantaneous batch size per GPU = %d", model_config.batch_size)
    print("  Gradient Accumulation steps = %d", model_config.gradient_accumulation_steps)
    ...
    for _epoch, _ in enumerate(train_iterator):
        epoch_iterator = trange(len(tr_dl), desc="Iteration")
        epoch = _epoch
        epoch_iterator = tqdm(tr_dl, desc="Iteration")
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % model_config.gradient_accumulation_steps == 0:
                ...
                if model_config.save_steps > 0 and global_step % model_config.save_steps == 0:
                    ...
                    output_dir = os.path.join(model_config.output_dir, 'aihub1_epoch-{}'.format(epoch + 1))
                    ...
                    if is_best:
                        ...
                        # print("test acc: %s, loss: %s, global steps: %s", str(eval_summary['eval_acc']), str(eval_summary['eval_loss']), str(global_step))

                        checkpoint_manager.save_checkpoint(state, 'aihub1-best-epoch-{}-step-{}-acc-{:.3f}.bin'.format(epoch + 1, global_step, best_dev_acc))
                        ...
                        cr_save_path = model_dir / 'aihub1-best-epoch-{}-step-{}-acc-{:.3f}-cr.csv'.format(epoch + 1, global_step, best_dev_acc)
                        cm_save_path = model_dir / 'aihub1-best-epoch-{}-step-{}-acc-{:.3f}-cm.png'.format(epoch + 1, global_step, best_dev_acc)
                        save_cr_and_cm(val_dl, list_of_y_real, list_of_pred_tags, cr_save_path=cr_save_path, cm_save_path=cm_save_path)
                    else:
                        torch.save(state, os.path.join(output_dir, 'aihub1-model-epoch-{}-step-{}-acc-{:.3f}.bin'.format(epoch + 1, global_step, eval_summary["eval_acc"])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', default='/home/hoya/notebooks/syoung21.yoo/data/base_model_with_crf_val', help="Directory containing config.json of model")
