...
from join_jamo import join_jamos
...
class WordpieceTokenizer(object):
	...
	def tokenize(self, text):
		...
		for token in whitespace_tokenize(text):
			...
			sub_tokens = []
			"""
			piece_list=korean_to_be_englished(text)
			print(piece_list)
			num_list=[]
			for p in piece_list:
				num=len(p)
			...
			while start < len(chars):
				cur_substr = None
				while start < end:
					substr = join_jamos(chars[start:end])
					#print(substr)
					### joonho.lim @ 2019-03-15
					# if start > 0:
						# substr = "##" + substr
